...
- **Dashboard logic:** date windows measure from *start of today* so a
  ...
  unchanged. `not_relevant` is the durable de-prioritization for roles
  outside the user's field (e.g. EE/CE hardware roles): deleting an
  imported row just resurrects it on the next feed pull, but the
  non-clobber rule means a status mark survives re-imports. Marked rows
  are excluded from every dashboard surface; in the opportunities table
  they stay visible but sink to the end under every sort (dimmed) —
  marking a row drops it to the bottom immediately via the optimistic
  re-sort. They double as labeled data for future filter refinement —
  don't repurpose `closed` (auto-set by ghost cleanup) for this.
